...
def google_callback(request):
    ...
    try:
        ...
        if social_user.provider != 'google':
            return JsonResponse({'error message': 'no matching social type'}, status=status.HTTP_400_BAD_REQUEST)

        # 이미 Google로 제대로 가입된 유저 => 로그인 & 해당 유저의 jwt 발급
        data = {'access_token': access_token, 'code': code}
        accept = requests.post(f"{BASE_URL}accounts/google/login/django", data=data)
        ...
        return JsonResponse({'error message': 'not registered as a social user'}, status=status.HTTP_400_BAD_REQUEST)


class GoogleToDjangoLogin(SocialLoginView): # if you want to use Authorization Code Grant, use this
